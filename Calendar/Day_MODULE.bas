...
Sub DrawMainEvents 
	...
	If Not(Main.CalendarMap.ContainsKey(currentDate)) Then
		Return
	End If
	
	Dim eventmap As Map = Main.CalendarMap.Get(currentDate)
...
Sub DrawTimelineEvents
	If Not(Main.CalendarMap.ContainsKey(currentDate)) Then Return
...
Sub hour_click
	...
	Dim eventmap As Map
	If Main.CalendarMap.ContainsKey(currentDate) Then
		eventmap = Main.CalendarMap.Get(currentDate)
	Else
...
Sub MapInitializer As Map
	...
	Main.CalendarMap.Put(currentDate, eventmap)
